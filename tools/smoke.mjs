...
section('take 119 — the surface beyond Home, the mode swipe, the last literal sizes onto the tokens');
{ const css = (html.match(/<style>([\s\S]*?)<\/style>/) || ['', ''])[1];
  const radii = c => [...c.matchAll(/border-radius:([^;}"]+)/g)].map(m => m[1].trim());
  const bare = v => !/^(var\(--r-(sm|md|lg|xl|pill)\)|var\(--r\)|var\(--t-r,16px\)|[234]px|50%|22px 22px 0 0|3px 3px 0 0|0)$/.test(v);
  ok('every radius in the stylesheet reads a token (--r-xl:18px is the surface\'s): the 2 to 4 px hairlines, the sheet\'s 22 px top and a circle\'s 50% are the literals left', /--r-xl:18px;/.test(css) && radii(css).length >= 50 && radii(css).filter(bare).length === 0, radii(css).filter(bare).join(', '));
  ok('...control: a 9 px corner planted in the sheet is caught', radii(css + '\n.probe119{border-radius:9px}').filter(bare).length === radii(css).filter(bare).length + 1);
  const shareAt = js.indexOf('<style>body{margin:0;padding:24px'), shareEnd = js.indexOf('</style></head><body>', shareAt);
  const share = shareAt >= 0 && shareEnd > shareAt ? js.slice(shareAt, shareEnd) : '', tpl = js.replace(share, '');
  ok('the script\'s templates read the tokens too: no literal corner but the spread bar\'s 3 px, no literal size but the one computed from a picture\'s width; the share page, a fixed page of its own, keeps its sheet', share.length > 100 && /h1\{font-size:28px;margin:0 0 4px;color:#C9A24A\}/.test(share) && radii(tpl).every(v => /^(var\(--r-(sm|md|pill)\)|\$\{radius|50%|3px)$/.test(v)) && (tpl.match(/font-size:\d+(?:\.\d+)?px/g) || []).length === 0 && /font-size:\$\{Math\.max\(12, Math\.round\(w \/ 4\)\)\}px/.test(tpl), radii(tpl).filter(v => !/^(var\(--r-(sm|md|pill)\)|\$\{radius|50%|3px)$/.test(v)).concat(tpl.match(/font-size:\d+(?:\.\d+)?px/g) || []).join(', '));
  ok('...control: a planted 13px template is caught', ((tpl + '<b style="font-size:13px">').match(/font-size:\d+(?:\.\d+)?px/g) || []).length === (tpl.match(/font-size:\d+(?:\.\d+)?px/g) || []).length + 1);
  ok('the picture helper\'s radius is a token string -- a card\'s the small one, a product\'s the mid one -- and the count pill is a pill (its 9 px corner was half its height)', /function picBox\(p, w, h, label, radius = 'var\(--r-md\)'\) \{/.test(js) && /border-radius:\$\{radius\};overflow:hidden;background:linear-gradient\(160deg,\$\{a\},var\(--card2\)\);/.test(js) && /esc\(\(p\.num \|\| ''\)\.split\('-'\)\.pop\(\) \|\| ''\), 'var\(--r-sm\)'\);/.test(js) && /\.fcount\{[^}]*border-radius:var\(--r-pill\);/.test(css));
  ok('the counter\'s big numbers read the total size, the curtain\'s prompt the head and hero sizes, a product\'s set code in its art\'s place the head size (44, 22, 34 and 28 px until take 119)', /<div class="big" style="font-size:var\(--fs-total\);line-height:1;margin:4px 0">\$\{pl\.life\}<\/div>/.test(js) && /<div class="big" style="font-size:var\(--fs-head\);margin-bottom:8px">Hand the phone to<\/div>/.test(js) && /<h2 style="font-size:var\(--fs-hero\);margin:0 0 18px">/.test(js) && /<div class="ph" style="font-family:var\(--display\);font-size:var\(--fs-head\)">/.test(js));
  /* the surface */
  ok('the premium surface is the rule for every panel of Collect and Hunt and Prep & Play (P1); Home\'s own rule is gone, and the plain panel rule still carries the edge and the padding the surface builds on', /\n\.panel\{background:linear-gradient\(180deg,var\(--card2\),var\(--card\)\);border-radius:var\(--r-xl\);margin-bottom:12px;box-shadow:inset 0 1px 0 rgba\(255,255,255,\.05\),0 10px 26px rgba\(0,0,0,\.28\)\}/.test(css) && !/#home \.panel\{/.test(css) && /\n\.panel\{background:var\(--card\);border:1px solid var\(--line\);border-radius:var\(--r\);\n  padding:16px;margin-bottom:16px\}/.test(css) && /#home \.hero\{[^}]*border-radius:var\(--r-xl\);/.test(css) && /\.sheetbody\{[^}]*border-radius:22px 22px 0 0;/.test(css));
  ok('the caps label and the fading rule on every panel title of Collect and Hunt, a count beside it in the body face, and on the distributor folds\' titles (S1); Local\'s two long labels are shorter', /\n\.panel h3\{display:flex;flex-wrap:wrap;align-items:center;gap:6px 10px;font-family:var\(--heavy\);font-weight:800;font-size:var\(--fs-cap\);letter-spacing:\.2em;text-transform:uppercase;color:var\(--dim\);margin-bottom:12px\}/.test(css) && /\n\.panel h3 \.note\{order:2;margin:0;letter-spacing:0;text-transform:none;font-family:var\(--body\);font-weight:600;white-space:nowrap\}/.test(css) && /\n\.dfold \.ttl\{font-family:var\(--heavy\);font-weight:800;font-size:var\(--fs-cap\);letter-spacing:\.2em;text-transform:uppercase;color:var\(--dim\)\}/.test(css) && /<h3>Shops that run events\$\{R \?/.test(js) && /<h3>Target stores checked <span class="note">/.test(js) && !/#home \.panel h3\{/.test(css));
  ok('the collection tile is a surface card, its picture edged and shadowed (T1)', /\n\.tile\{background:linear-gradient\(180deg,var\(--card2\),var\(--card\)\);border:1px solid var\(--line\);border-radius:var\(--r-xl\);/.test(css) && /\n\.tile \.art\{border:1px solid var\(--line\);box-shadow:0 6px 14px rgba\(0,0,0,\.35\)\}/.test(css));
  ok('the tile\'s picture reads the mid radius (9 px until take 119)', /\n\.art\{aspect-ratio:\.716;border-radius:var\(--r-md\);/.test(css));
  /* the swipe */
  ok('the mode switch is a slide (take 119): the screen arriving comes in from the finger\'s side over the one leaving, which keeps its palette and slides a third of the way out, dimming', /\n\.screen\.out\{display:block;position:fixed;left:0;right:0;top:0;z-index:var\(--z-screen\);pointer-events:none;overflow:hidden;animation:modeOutR var\(--dur-ui\) cubic-bezier\(\.4,0,\.2,1\) forwards\}/.test(css) && /:root\.swap-l \.screen\.out\{animation-name:modeOutL\}/.test(css) && /:root\.mode-swap\.swap-l \.screen\.on\{animation-name:modeInL\}/.test(css) && /:root\.mode-swap\{overflow-x:hidden\}/.test(css) && /:root\.mode-swap body\{overflow-x:clip\}/.test(css) && ['modeInR', 'modeInL', 'modeOutR', 'modeOutL'].every(k => css.includes('@keyframes ' + k + '{')) && /@keyframes modeOutR\{from\{transform:translate\(0,var\(--out-top,0px\)\);opacity:1\}to\{transform:translate\(-30%,var\(--out-top,0px\)\);opacity:\.5\}\}/.test(css) && /@keyframes modeInR\{from\{transform:translateX\(100%\);background:var\(--bg\);/.test(css) && !/@keyframes modeIn\{/.test(css));
  ok('...it runs from a tap or a swipe only -- MODE.set itself stays instant for the guide, Back and the harness -- and the leaving screen is taken out when its animation ends, or at 400 ms', /function slideMode\(to\) \{/.test(js) && /const MODE_ORDER = \['collect', 'play', 'hunt'\];/.test(js) && /leaving\.style\.cssText = `--out-top:\$\{outTop\}px;background:var\(--bg\);\$\{toks\}`; leaving\.classList\.add\('out'\); MODE\._out = leaving;/.test(js) && /leaving\.addEventListener\('animationend', finishSlide, \{ once: true \}\)/.test(js) && /&& !reducedMotion\(\)\) \{/.test(js) && /if \(MODE\._swiped\) return;/.test(js) && /slideMode\(b\.dataset\.mode\);/.test(js) && !/MODE\.set\(b\.dataset\.mode\)/.test(js) && /set\(m, navigate = true\) \{/.test(js) && /this\.cur = m; saveJson\('vault\.mode', m\);/.test(js));
