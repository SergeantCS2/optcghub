...
*Current as of take 117.* Paste the block between the rules into a new session
...
built across 117 takes by previous sessions. The repo is
...
- **For the UI/UX session (the owner, take 115).** It drafted while
  ...
  and take 117 (the owner's polish list and the audit's fixes) are its
  first two, each in HANDOFF; take 118 (Collect on indigo and gold, Home's
  premium pass, Hunt in kraft) is ported onto the same baseline and
  follows, one take one PR like any other; take 119's previews (the
  surface beyond Home, the tiles, Play, the mode swipe) are with the owner. The owner's words for it: it "will be
...
Say "take 118" and begin with PROTOCOL §0.
