# V1-STATE — what exists, as of take 113

*Current as of take 113.* The honest inventory: sorted into what is PROVEN on a
...
| Packaging | signed APK (committed sideload key), AAB branch for Play (needs 4 secrets), the adaptive icon (background, foreground, monochrome), legacy and round icons, the reminders' glyph, the splash and the Play icon from five SVGs (`ci/icon.py`, checked, its controls in the gate; take 113, the owner's pick), CAMERA + POST_NOTIFICATIONS + AD_ID in the manifest | built every take since 9; signer verified by aapt2/apksigner |
