...
*Current as of take 115.* Everything else is decided and recorded in AGENDA.
...
**D11 — AdMob account.** *Take 33: it exists — publisher `pub-6243777967151950`, and the app-ads.txt line is in RUNBOOK-play §9. Take 41: the app ID `ca-app-pub-6243777967151950~1538944343` is in `config.py`; the two rewarded ad units are still Google's test units.* *Take 115: needed now -- the app has been live on Google Play since 24 Sept, and every install shows Google's test ads, which earn nothing.* Two values into `tools/config.py`: the **two rewarded ad-unit IDs** (`ca-app-pub-6243777967151950/…`), one for scan credits and one for deck saves. The app ID is already there. The ad settings ride the Pages manifest, so the take that carries them switches every install at its next sync, sideloads and older takes included: register the owner's own devices as AdMob test devices first. No consent (UMP) flow exists yet (AGENDA A43).
